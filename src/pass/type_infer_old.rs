...
pub struct GlobalEnv (pub HashMap<Id, (ModuleEnv, Module)>);
...
impl TypeCheck<&mut GlobalEnv> for Module {
    fn type_check(&mut self, pm: &mut GlobalEnv) {
        let mut old_m_pm = if pm.0.contains_key(&self.id) {
            ...
            (ModuleEnv(HashMap::new()), self.clone())
        };
        let mut new_m_pm = old_m_pm.clone();

        self.ports.type_check(&mut new_m_pm.0);
        ...
        while new_m_pm != old_m_pm {
            ...
            self.stmts.type_check((&mut new_m_pm.0, pm));
        }
        //  */
    ...
    fn type_check(&mut self, (pm, g_pm): (&mut ModuleEnv, &mut GlobalEnv)) {
        ...
        match &mut self.raw_stmt {
            ...
            RawStmt::Inst(module_name, inst_name) => {
                let (_, modu) = g_pm.0.get(module_name).unwrap();
                if modu.is_uninstenced() {
                    // fork new module
                    let mut modu = modu.clone();
                    // gen name
                    let new_id = format!("{}$${}", modu.id, gen_id());
                    // rename
                    modu.id = new_id.clone();
                    *module_name = new_id;
                    // module instence
                    modu.type_check(g_pm);
                }
            },
            RawStmt::Node(s, a) => {
            ...
            RawStmt::Connect(a, b) => {
                ...
                let rt = a_ty.clone().map(|a| b_ty.clone().map(|b| a.unify(&b))).flatten().flatten();
                if let Some(rt) = rt {
                    a.set_type((pm, g_pm), &rt);
                    b.set_type((pm, g_pm), &rt);
                }
            },
...
impl TypeInference<(&mut ModuleEnv, &mut GlobalEnv)> for Id {
    fn type_infer(&self, (pm, g_pm): (&mut ModuleEnv, &mut GlobalEnv)) -> Option<Type> {
        ...
        g_pm.0.get(self).map(|x| x.0.type_infer(self)).flatten()
    }
}
